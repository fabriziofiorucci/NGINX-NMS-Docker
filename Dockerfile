...
# Initial setup
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y -q build-essential git nano curl jq wget gawk lsb-release rsyslog systemd && mkdir -p deployment/setup

# NGINX Instance Manager 2.0
COPY $NIM_DEBFILE /deployment/setup/nim.deb
COPY ./container/startNIM.sh /deployment/
RUN chmod +x /deployment/startNIM.sh && wget https://docs.nginx.com/nginx-instance-manager/scripts/fetch-external-dependencies.sh -qO /deployment/setup/fetch-external-dependencies.sh
...
RUN bash /deployment/setup/fetch-external-dependencies.sh ubuntu20.04 && \
	...
	dpkg -i ./*.deb && rm *.deb && rm /etc/nginx/conf.d/default.conf
...
RUN if [ "$BUILD_WITH_COUNTER" = "true" ] ; then apt-get install -y python3-pip python3-dev python3-simplejson && \
	...
	cp F5-Telemetry-Tracker/f5tt/*.py . && \
